...
你必须只返回一个 JSON 对象，顶层键固定为 data，其值为指令数组：{"data": [ ... ]}。即使只有一条指令，也必须放在 data 数组中：{"data": [{"order":"capture",...}]}。数组中的指令按顺序执行。setCurrentDate（时间跳跃）必须放在 data 数组的最后一项，作为该时间步的收尾。不要输出多余解释文字。所有地点都用城市中文名填写即可，无需任何编码。
...
11. setCurrentDate — 推进全局日期（必须放 data 数组最后一项）
...
批量指令（data 数组按顺序执行，setCurrentDate 放在最后）：
{"data":[
